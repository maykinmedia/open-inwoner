...
from open_inwoner.djangocms_4_migration.models import PageData
...
def create_new_alias_plugin(old_plugin, new_alias_grouper, is_src_plugin=False):
    ...
    if new_plugin:
        ...
        logger.info("Deleting old plugin", plugin_id=old_plugin_id)
        old_plugin.delete()
        ...
        if child_plugins:
            for plugin in child_plugins:
                ...
                if second_level_child_plugins:
                    plugins.extend(second_level_child_plugins)
...
def _process_sites(plugin_id_list):
    for site in Site.objects.all():
        # Sites Placeholders
        logger.info("Processing site", site_id=site.id)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        with transaction.atomic():
            ...
            logger.info("old stats", stats=old_stats)
            logger.info("new stats", stats=new_stats)
