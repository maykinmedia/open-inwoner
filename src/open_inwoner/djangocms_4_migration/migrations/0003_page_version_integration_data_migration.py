...
def forwards(apps, schema_editor):
    ...
    # Get USERNAME_FIELD from the actual User model (not historical)
    # Default to email for Open Inwoner
    USERNAME_FIELD = getattr(get_user_model(), "USERNAME_FIELD", "email")
    ...
    def _create_version(page_content, state=DRAFT, number=1):
        ...
        try:
            ...
            logger.warning(
                "User not found, falling back", created_by=page_content.page.created_by
            )
            created_by, created = get_or_create_migration_user(user_model=User)

        logger.info("Creating version for new title", page_content_pk=page_content.pk)
    ...
    for existing_title in PageData.objects.using(db_alias).all():
        ...
        """
        logger.info("Existing title", title_id=existing_title.title_id)
...
class Migration(migrations.Migration):
    dependencies = [
        ...
        (
            "cms",
            "0034_remove_pagecontent_placeholders",
        ),  # Run after the CMS4 migrations
        (
            "djangocms_versioning",
            "0015_version_modified",
        ),  # Ensure the modified field is present
    ]
