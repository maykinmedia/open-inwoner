...
class QuestionnaireStep(MPTTModel):
    ...
    """
    parent = TreeForeignKey('self', verbose_name=_('Vorige stap'),
                            help_text=_('Geeft aan op welke stap dit een vervolgstap is.'), on_delete=models.CASCADE,
                            blank=True, null=True)
    parent_answer = models.CharField(_('Antwoord op vorige vraag'),
                                     help_text=_('Dit label wordt getoond bij de keuzes van de vorige stap.'),
                                     max_length=255,
                                     blank=True)

    title = models.CharField(_('Titel'),
                             help_text=_('Titel van deze stap, wordt overgenomen van de hoofdstap indien leeggelaten.'),
                             max_length=255, blank=True)
    description = models.CharField(_('Beschrijving'),
                                   help_text=_(
                                       'Deze tekst wordt ter ondersteuning onder de titel getoond, wordt overgenomen van de hoofdstap indien leeggelaten.'),
                                   max_length=255, blank=True)

    question = models.CharField(_('Vraag'), help_text=_('De stelling of vraag'), max_length=255)
    slug = models.SlugField(_('URL vriendelijke naam'), max_length=255)
    help_text = models.CharField(_('Ondersteunende tekst'), help_text=_('Beschrijvende tekst bij de vraag.'),
                                 default=_('Kies het antwoord dat het meest van toepassing is'), max_length=510)

    content = models.TextField(_("Uitgebreide informatie"), help_text=_('Deze inhoud wordt weergegeven in deze stap.'),
                               blank=True)
    ...
    def get_absolute_url(self) -> str:
        ...
        root = self.get_root()
        return reverse('questionnaire:descendent_step', kwargs={"root_slug": root.slug, "slug": self.slug})
