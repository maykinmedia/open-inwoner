...
def build_cms_search(query_str: str) -> Search:
    ...
    search = search.query(
        Q(
            "multi_match",
            query=query_str,
            fields=["title", "description", "rendered_content_placeholder"],
        )
    )
    search = search.highlight("title", "rendered_content_placeholder", fragment_size=50)
    return search


def search_products(query_str: str, filters=None) -> ProductSearchResult:
    """Execute a search across the Products index."""
...
def multi_search(
    ...
    search_query = search_query.add(page_search)
