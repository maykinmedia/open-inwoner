...
from open_inwoner.mijn_aanvragen.import_export import (
    ZGWConfigExport,
    ZGWConfigImport,
    ZGWImportError,
)
...
class TestCatalogusImport(TestCase):
    ...
    def test_import_jsonl_update_reports_duplicate_natural_keys_in_upload_file(self):
        ZGWExportImportMockData(with_dupes=True)
    ...
    def test_import_jsonl_fails_with_catalogus_domein_rsin_mismatch(self):
        ...
        with self.assertLogs(
            logger="open_inwoner.mijn_aanvragen.import_export", level="ERROR"
        ) as cm:
