...
class ZGWImportTest(ClearCachesMixin, TestCase):
    ...
    def test_import_zaaktype_configs_multiple_api_groups_no_false_positives(self, m):
        ...
        CatalogMockData(root1).install_mocks(m)
        CatalogMockData(root2).install_mocks(m)

        ZaakTypeMockData(root1).install_mocks(m)
        ZaakTypeMockData(root2).install_mocks(m)
