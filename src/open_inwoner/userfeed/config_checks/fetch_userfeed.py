...
class FetchUserfeedForm(forms.Form):
    """
    Intentionally has no fields: the check always runs for the ``User`` the
    admin page it was launched from belongs to, see ``FetchUserfeedCheck.run``.
    """
    ...
    def run(
        ...
        if user is None:
            return GenericConfigCheckResult(
                ...
                message=_("This check can only be run from a user's admin page"),
            )
