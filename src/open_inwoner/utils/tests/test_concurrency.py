...
class TimedParallelTests(SimpleTestCase):
    def test_as_completed_yields_results_when_nothing_is_cancelled(self):
        ...
        self.assertFalse(result.has_cancelled_futures)
        self.assertEqual(result.cancelled_futures, set())
        self.assertFalse(result.has_completed_late_futures)
    ...
    def test_completed_and_completed_late_are_tracked_separately(self):
        ...
        self.assertTrue(result.has_completed_late_futures)
        self.assertEqual(result.cancelled_futures, set())
        self.assertFalse(result.has_cancelled_futures)
    ...
    def test_queued_futures_are_cancelled_while_running_ones_are_still_yielded(self):
        ...
        and it should *not* end up in ``cancelled_futures``.
        """
        started = threading.Event()
        release = threading.Event()
        timer = threading.Timer(_RELEASE_DELAY, release.set)
        ...
        self.assertEqual(result.cancelled_futures, set(queued))
        self.assertTrue(result.has_cancelled_futures)
    ...
    def test_records_completed_metric_for_every_future_when_nothing_is_cancelled(
        self,
    ):
        """
        The metric is recorded on every call, not just ones that hit the
        deadline, so a cancellation rate can be computed against a real
        denominator later.
        """
        with patch(
    ...
    def test_records_one_completed_call_metric_when_nothing_is_cancelled(self):
        """
    ...
    def test_incomplete_accounting_raises_instead_of_returning_bad_data(self):
        ...
        this class missing `self._cancelled.update(not_yet_started)` would
        ...
        try:
            with TimedParallel(max_workers=1, name=_NAME) as executor:
                ...
                result._cancelled = NoOpUpdateSet()
    ...
    def test_exit_cancels_futures_never_passed_to_as_completed(self):
        """
        ``as_completed()`` is what populates ``cancelled_futures``, but the
