...
class TestPartialUpdateDeactivate(APITestCase):
    def setUp(self):
        self.user = UserFactory(deactivated_on=None, is_active=True, is_staff=False)
    ...
    def test_endpoint_updates_user(self):
        ...
        self.assertEqual(
            response.json(), {"detail": _("Your account has been deactivated.")}
        )
    ...
    def test_endpoint_does_not_update_staff_user(self):
        user = UserFactory(deactivated_on=None, is_active=True, is_staff=True)
        ...
        self.assertEqual(
            response.json(), {"detail": _("Your account cannot be deactivated.")}
        )
