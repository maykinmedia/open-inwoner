...
class InclusionTagWebTest(WebTest):
    ...
    def get_args(self, args: dict) -> str:
        ...
        *names*, not the values, end up in the template string. Mirrors the fix for
        `RenderableTag` in `open_inwoner.components.utils` (CodeQL alert #60).
        ...
        Returns: A str with variable assignments in a format suitable for template rendering
        """
        return " ".join(f"{k}=template_context_{k}" for k in args)
    ...
    def get_template_context(self, args: dict) -> dict:
        ...
        """
        return {f"template_context_{k}": v for k, v in args.items()}
