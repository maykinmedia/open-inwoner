...
class SideNavMenuData:
    ...
    def _extract_icon(self, node: NavigationNode) -> str | None:
        ...
        try:
            ...
            # In CMS 4 there is one Page object per logical page; look up icon directly
    ...
    def _is_visible_to_user(self, node: NavigationNode) -> bool:
        try:
            ...
            has_published = PageContent._original_manager.filter(
                page=page, language=current_language, versions__state=PUBLISHED
            ).exists()
            ...
            if is_staff:
                ...
                # Allow staff to see pages that have draft content in the current
                # language (e.g. when browsing in edit mode).
                has_draft = PageContent._original_manager.filter(
                    page=page, language=current_language, versions__state=DRAFT
                ).exists()
                if has_draft:
                    return True
...
def react_sidenav_data(context):
    ...
    Cached at request-level to ensure the sidenav is computed once per request
    """
    try:
    ...
    if not hasattr(request, "_sidenav_menu_data"):
        try:
            request._sidenav_menu_data = SideNavMenuData(
                context, root_id="home"
            ).get_menu_data()
        except Exception:
            logger.exception("Error loading sidenav menu")
            request._sidenav_menu_data = []
    return request._sidenav_menu_data


# views that aren't part of normal navigation: either a forced step the user
# hasn't completed yet, or otherwise voluntarily revisited outside the regular
# flow (e.g. switching eHerkenning branch)
