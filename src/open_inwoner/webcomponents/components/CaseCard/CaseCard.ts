...
class ExternalLinks extends LitElement {
  ...
  render() {
    ...
    return html`
      <section class="plugin case-cards">
        ${this.title
          ? html`<h2 class="utrecht-heading-2">${this.title}</h2>`
          : ''}

        <ul class="card-container card-container--columns-2 case-cards__list">
          ${this.links.map(
            (link) => html`
              <li class="case-cards__list-item">
                <a
                  href="${link.url}"
                  class="case-card"
                ...
                >
                  <span class="case-card__custom-icon">
