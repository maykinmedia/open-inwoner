...
def get_published_page_ids():
    """Return a queryset of Page PKs that have at least one published version."""
    return PageContent._original_manager.filter(versions__state=PUBLISHED).values_list(
        "page_id", flat=True
    )
