...
class CMSPagesConfigurationStep(BaseConfigurationStep):
    ...
    def _sync_redirect_pages(
        ...
        """
        enabled_configs = {
            model_attr: page_config
            for model_attr, reverse_id in _REDIRECT_PAGES.items()
            if (page_config := getattr(model, model_attr)) is not None
            and page_config.enabled
        }
        if not enabled_configs:
            return

        # The apphook pages these redirect to may have been created moments ago in
        # the loop above. Django CMS only recomputes its apphook URL patterns on an
        # actual HTTP request (via ApphookReloadMiddleware); in a management command
        # there's no request, so if `cms.urls` was already imported earlier in this
        # process (e.g. by a preceding setup_configuration step reversing an
        ...
        # reimport of the urlconf so the new apphook pages' namespaces are picked up.
        # Once per run is enough: nothing invalidates it again between here and the
        # end of the loop below.
        reload_urlconf()

        for model_attr, page_config in enabled_configs.items():
            reverse_id = _REDIRECT_PAGES[model_attr]

            try:
                redirect_url = reverse(page_config.redirect_url_name)
            except NoReverseMatch as exc:
                raise ConfigurationRunFailed(
                    f"Unable to reverse `{page_config.redirect_url_name}` for the "
                    f"`{model_attr}` redirect page. If it belongs to an apphook, "
                    "make sure that apphook's page is enabled in this same "
                    "configuration."
                ) from exc
