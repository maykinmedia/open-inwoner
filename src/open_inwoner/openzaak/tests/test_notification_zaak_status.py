...
class StatusNotificationHandlerTestCase(
    ...
    def test_status_bails_when_no_roles_found_for_case(self, m, mock_handle: Mock):
        ...
        result = handle_zaken_notification(data.status_notification)
    ...
    def test_status_bails_when_cannot_fetch_status_history(self, m, mock_handle: Mock):
        ...
        self.assertEqual(
            ...
            NotificationResult(
                status=NotificationStatus.DROPPED,
                sent_count=0,
                reason="cannot retrieve status_history",
            ),
        )
        mock_handle.assert_not_called()
        self.assertTimelineLog(
    ...
    def test_status_bails_when_status_history_is_single_initial_item(
        ...
        self.assertEqual(
            ...
            NotificationResult(
                status=NotificationStatus.DROPPED,
                sent_count=0,
                reason="skip initial status notification",
            ),
        )
        mock_handle.assert_not_called()
        self.assertTimelineLog(
    ...
    def test_status_bails_when_cannot_fetch_status_type(self, m, mock_handle: Mock):
        ...
        self.assertEqual(
            ...
            NotificationResult(
                status=NotificationStatus.DROPPED,
                sent_count=0,
                reason=f"cannot retrieve status_type {data.status_type_final['url']}",
            ),
        )
        mock_handle.assert_not_called()
        self.assertTimelineLog(
    ...
    def test_status_bails_when_status_type_not_marked_as_informeren(
        ...
        self.assertEqual(
            ...
            NotificationResult(
                status=NotificationStatus.DROPPED,
                sent_count=0,
                reason="status_type.informeren is false",
            ),
        )
        mock_handle.assert_not_called()
        self.assertTimelineLog(
    ...
    def test_status_bails_when_skip_informeren_is_set_and_no_zaaktypeconfig_is_found(
        ...
        self.assertEqual(
            ...
            NotificationResult(
                status=NotificationStatus.DROPPED,
                sent_count=0,
                reason=f"'skip_notification_statustype_informeren' is True but cannot retrieve zaaktype configuration '{data.zaak_type['identificatie']}'",
            ),
        )
        mock_handle.assert_not_called()
        self.assertTimelineLog(
    ...
    def test_status_handle_notification_status_type_config_notify_false(
        ...
        self.assertEqual(
            ...
            NotificationResult(
                status=NotificationStatus.DROPPED,
                sent_count=0,
                reason="'notify_status_change' is False for the status type configuration",
            ),
        )
        mock_handle.assert_not_called()
    ...
    def test_status_bails_when_skip_informeren_is_set_and_zaaktypeconfig_is_found_but_not_set(
        ...
        self.assertEqual(
            ...
            NotificationResult(
                status=NotificationStatus.DROPPED,
                sent_count=0,
                reason=f"zaaktype configuration '{data.zaak_type['identificatie']}' found but 'notify_status_changes' is False",
            ),
        )
        mock_handle.assert_not_called()
        self.assertTimelineLog(
    ...
    def test_user_status_notifications_disabled(self, m, mock_handle: Mock):
        ...
        self.assertEqual(
            ...
            NotificationResult(
                ...
                sent_count=0,
                reason="all 1 user(s) skipped: notifications disabled or no contact email",
            ),
        )
        mock_handle.assert_not_called()
