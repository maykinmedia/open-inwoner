...
class ZGWService:
    ...
    def get_raw_zaken(
        ...
        if result.has_cancelled_futures:
            logger.warning("Cancelled pending raw zaken fetches")
            raw_fetch_incomplete = True
    ...
    def get_visible_zaken(
        ...
        if result.has_cancelled_futures:
            logger.warning("Cancelled pending raw zaken fetches for group")
            raw_fetch_incomplete = True
        ...
        if result.has_cancelled_futures:
            logger.warning("Cancelled pending zaaktype resolutions")
            skipped.extend(
                SkippedZaak(
                    zaak_url=zaak_with_group.zaak.url,
                ...
                if future in result.cancelled_futures
            )
    ...
    def fully_resolve_zaken(
        ...
        if result.has_cancelled_futures:
            logger.warning("Cancelled pending zaak resolutions")
        timed_out_ids = {
            id(future_to_zaak[future]) for future in result.cancelled_futures
        }
    ...
    def get_formulieren(
        ...
        if result.has_cancelled_futures:
            logger.warning("Cancelled pending formulieren fetches")
        ...
        return FormulierenResult(
            formulieren=subs_with_api_group, timed_out=result.has_cancelled_futures
        )
    ...
    def get_zaak_by_uuid(self, uuid: str) -> ZaakWithApiGroup | None:
        ...
        if result.has_cancelled_futures:
            logger.warning("cancelled pending zaak-by-uuid fetches", uuid=uuid)
